...
if os.path.exists(csv_file):
    ...
    palette = ["#7eb0d5", "#b2e061", "#bd7ebe", "#ffb55a", "#ffee65", "#beb9db", "#fdcce5", "#8bd3c7"]
    ...
    for _, row in df.iterrows():
        ...
        if s not in added_nodes:
            net.add_node(s, label=s, color=group_color, size=10 + (degree_counts.get(s, 1) * 2))
            added_nodes.add(s)
            
        if t not in added_nodes:
            net.add_node(t, label=t, color=group_color, size=10 + (degree_counts.get(t, 1) * 2))
            added_nodes.add(t)
